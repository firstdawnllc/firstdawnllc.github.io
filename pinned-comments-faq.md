...
For issue based automations, you can use `{% raw %}`{{issue.key}}`{% endraw %}` to get the `Issue key` of the current issue the rule is acting on. {% raw %}`{{issue.comments.last.id}}`{% endraw %} will get the ID of the issue's most recent comment, but you can also use [other smart values to select different comments](https://support.atlassian.com/cloud-automation/docs/jira-smart-values-issues/#--issue.comments--). Automation replaces those placeholders with real values when the rule runs, so the action pins the latest comment on the triggering issue. You can learn more about Jira Automation smart values [here](https://support.atlassian.com/cloud-automation/docs/smart-values-in-jira-automation/).
